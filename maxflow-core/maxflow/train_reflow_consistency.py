...
from maxflow.utils.quality_assessment import calculate_consistency
...
def train_reflow(
    ...
    accelerator.print(f"🚀 Reflow Distillation (Consistency) on {device} | Precision: {mixed_precision}")
    ...
    try:
        ...
        accelerator.print("✅ Student initialized from Teacher weights")
    except Exception as e:
        accelerator.print(f"⚠️ Could not load checkpoint ({e}). Initializing from scratch.")
        student = MaxFlow(node_in_dim=58, hidden_dim=args.hidden_dim, num_layers=args.num_layers)
    ...
    if teacher_checkpoint:
        try:
            ...
            teacher.eval()
            accelerator.print("✅ Teacher model loaded for consistency distillation")
        except Exception as e:
            accelerator.print(f"⚠️ Could not load teacher checkpoint ({e}). Consistency distillation disabled.")
    ...
    for epoch in range(epochs):
        ...
        if accelerator.is_main_process:
            ...
            print(f"📈 Avg MSE Loss: {total_mse_loss / len(loader):.4f}")
            print(f"📈 Avg Consistency Loss: {total_consistency_loss / len(loader):.4f}")
            ...
            if (epoch + 1) % 5 == 0 or epoch + 1 == epochs:
                ...
                print(f"💾 Checkpoint saved to {save_dir}/reflow_epoch_{epoch+1}.pt")
    ...
    print(f"🎉 Training complete! Final model saved to {save_dir}/reflow_final.pt")
